<template>
  <div>
    <table class='ranking-table table-auto max-md:w-full' v-if="!loadingCharacters" style="overflow: scroll">
      <thead>
        <tr>
      ...
      <tbody>
        <tr v-for="(character, index) in pageCharacters" :key="character.id">
          ...
          <td><a :href="`/user/${character.nickname}`">{{ character.user__username }}</a></td>
          <td><a :href="`/user/${character.nickname}`">{{ character.nickname }}</a></td>
