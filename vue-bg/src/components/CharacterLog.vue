<template>
  <div>
    ...
    <!-- Pop-up -->
      <div v-if="isModalOpen" class="fixed inset-x-0 bottom-20 flex items-end justify-end right-5 z-50 h-screen">
          <div class="bg-black w-1/5 border-2 border-green-400 p-4 relative resize overflow-auto" style="height: 50%">
            <ol class="relative border-l border-gray-200 dark:border-gray-700 h-full overflow-auto flex-grow">
              ...
              <li v-else v-for="action in actions" :key="action" class="mb-10 ml-6">            
                <h3 class="items-center text-center mb-1 text-lg font-semibold text-green-400 dark:text-white">{{ action.action__name }}</h3>
